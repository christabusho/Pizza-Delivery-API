...
async def place_an_order(order:OrderPlacementModel, Authorize:AuthJWT=Depends()):
...
async def update_order(id:int,order:OrderPlacementModel,Authorize:AuthJWT=Depends()):
    try:
        Authorize.jwt_required()
    except Exception as e:
        raise HTTPException(
